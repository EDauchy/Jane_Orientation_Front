...
import {
  RIASEC_ITEM_IDS,
  type RiasecItemId,
} from "../../../lib/quiz/validators/riasec";
...
export function Riasec({ moduleNumber, onReady }: ModuleProps) {
  ...
  const [cursor, setCursor] = useState(() => {
    const allAnswered = RIASEC_ITEM_IDS.every(
      (id) => id in (existing?.responses ?? {}),
    );
    if (allAnswered) return 0;
    const idx = RIASEC_ITEM_IDS.findIndex(
      (id) => !(id in (existing?.responses ?? {})),
    );
    return idx === -1 ? 0 : idx;
  });
  ...
  return (
    <div className="flex flex-col gap-5">
      <ModuleHeader
        ...
        title={
          <>
            18 activités, <Highlight color="yellow">trois réactions</Highlight>
          </>
        }
      />
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Card
            variant="soft"
            padding="lg"
            className="flex flex-col gap-2 min-h-[160px] justify-center"
          >
      ...
      <div className="flex items-center justify-between">
        ...
        {!allDone ? (
          <span className="text-[11px] text-muted">
            {order.length - answeredCount} item
            {order.length - answeredCount > 1 ? "s" : ""} restant
            {order.length - answeredCount > 1 ? "s" : ""}
          </span>
        ) : (
