...
export default function ProDashboard({ profile }: ProDashboardProps) {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const { data: reviews } = await supabase
          .from('reviews')
          .select('*, user_a:profiles!user_a_id(first_name, last_name), appointment:appointments(date)')
          .eq('user_b_id', profile.id)
        ...
        if (appointmentsData && reviews) {
          ...
          const declinedCount = appointmentsData.filter(a => a.status === 'CANCELLED' && a.user_b_id === profile.id).length;
          ...
          appointmentsData
            .filter(a => ['CONFIRMED', 'CANCELLED', 'COMPLETED'].includes(a.status))
            .slice(0, 5)
            .forEach(apt => {
  ...
  const getActivityColor = (status?: string) => {
    switch (status) {
      case 'confirmed': return { bg: 'bg-green-50', border: 'border-green-200', dot: 'bg-green-500' };
      case 'cancelled': return { bg: 'bg-red-50', border: 'border-red-200', dot: 'bg-red-500' };
      case 'completed': return { bg: 'bg-blue-50', border: 'border-blue-200', dot: 'bg-blue-500' };
      case 'review': return { bg: 'bg-yellow-50', border: 'border-yellow-200', dot: 'bg-yellow-500' };
      default: return { bg: 'bg-gray-50', border: 'border-gray-200', dot: 'bg-gray-500' };
    }
  };
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1 text-white">
                ...
                <div>
                  ...
                  <p className="text-xl text-white/90 mb-2 font-medium">
                    ...
                    {currentProfile.details?.years_experience && (
                      <span className="text-white/70 ml-2">• {currentProfile.details.years_experience} ans d'expérience</span>
                    )}
                  </p>
                </div>
                <button
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-white/70">({stats.totalReviews} avis)</span>
              </div>
