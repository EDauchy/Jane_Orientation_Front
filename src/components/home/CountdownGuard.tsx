...
const CountdownGuard = ({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl shadow-2xl ${containerClass}`}
    >
      {/* 1. LE CONTENU RÉEL */}
      ...
      {!isExpired && (
        <div className="absolute inset-0 z-10 bg-white/10 backdrop-blur-sm flex flex-col items-center justify-center border border-white/10">
          <span
            className={`${textColorClass} text-[10px] md:text-xs uppercase tracking-[0.4em] mb-6 font-bold drop-shadow-lg`}
          >
            Disponible dans
          </span>
          ...
          <div className="flex gap-3 md:gap-5">
            {Object.entries(timeLeft).map(([unit, value]) => (
              <div key={unit} className="flex flex-col items-center">
                ...
                <div
                  className={`${bgColorClass} ${borderColorClass} backdrop-blur-lg border w-12 h-14 md:w-16 md:h-20 flex items-center justify-center rounded-xl shadow-2xl`}
                >
                  <span
                    className={`text-xl md:text-3xl font-black ${textColorClass} drop-shadow-md`}
                  >
                    {String(value || 0).padStart(2, "0")}
                  </span>
                </div>
                <span
                  className={`text-[10px] mt-2 uppercase ${textColorClass} font-semibold drop-shadow-sm opacity-80`}
                >
                  {unit}
                </span>
              </div>
