...
export default function FindPro({
    ...
    const fetchPros = async () => {
      try {
        const res = await fetch(
          `/api/professionals?job=${encodeURIComponent(job)}`,
        );
        if (res.ok) {
  ...
  const getAvailableDaysText = (pro: Professional) => {
    const availability = pro.user_b_details.availability;
    if (!availability || Object.keys(availability).length === 0)
      return "Disponible tous les jours";
  ...
  const getBookingStatus = (proId: string) => {
    ...
    const professionCount = userAppointments.filter(
      (a) =>
    ...
    if (professionCount >= 2)
      return {
        disabled: true,
        reason: "Limite de 2 RDV atteinte pour ce métier",
      };

    const hasActiveWithPro = userAppointments.some(
      (a) =>
        a.user_b_id === proId &&
  ...
  const bookAppointment = async (proId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToast({
          message: `Erreur lors de la réservation: ${error.error || "Erreur inconnue"}`,
          type: "error",
        });
        setTimeout(() => setToast(null), 3000);
      }
        ...
        <Box sx={modalStyle}>
          ...
          <div className="flex justify-between items-center p-6 border-b border-gray-200 shrink-0">
            <div>
              <h2
                id="find-pro-title"
                className="text-2xl uppercase font-bold text-primary"
              >
            ...
            {loading || loadingAppointments ? (
              ...
              <div className="grid gap-6">
                {pros.map((pro) => {
                  ...
                  const datePart =
                    selectedPro === pro.id && selectedDate
                      ? selectedDate.split("T")[0]
                      : "";
                  const isAvailableDay = datePart
                    ? isDateAvailable(datePart, pro)
                    : true;
                  const availableHours = datePart
                    ? getAvailableHours(datePart, pro)
                    : [];
                            ...
                            <p className="text-sm text-gray-600 mt-0.5">
                              ...
                              {pro.user_b_details.years_experience && (
                                <span className="text-gray-400 ml-2">
                                  • {pro.user_b_details.years_experience} ans
                                  d'exp.
                                </span>
                              )}
                            </p>
                          ...
                          <div className="flex gap-2">
                            <div className="flex-1">
                              ...
                              <AvailableDatePicker
                                ...
                                selectedDate={
                                  isThisProSelected ? datePart || null : null
                                }
                                onDateSelect={(date) => {
                            ...
                            <div className="w-32">
                              <SelectInput
                                ...
                                onChange={(e) => {
                                  ...
                                  setSelectedDate(
                                    `${date}T${e.target.value}:00:00.000Z`,
                                  );
                                }}
                                disabled={
                                  disabled ||
          ...
          <div className="border-t border-gray-200 p-4 bg-gray-50 rounded-b-2xl shrink-0">
            <p className="text-center">
              💡 Sélectionnez une date et une heure pour confirmer votre
              rendez-vous
            </p>
          </div>
        </Box>
