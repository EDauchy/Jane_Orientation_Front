...
export default function Dashboard() {
  ...
  const hasTestResults =
    isUserA &&
    profile.details?.test_results &&
    profile.details.test_results.length > 0;
                ...
                {!hasTestResults ? (
                  <div className="bg-white shadow-sm rounded-lg p-6 text-center">
                    ...
                    <p className="mt-2 text-gray-500">
                      Commencez par passer le test d'orientation.
                    </p>
                    <button
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                      {profile.details.test_results.map(
                        (job: string, idx: number) => (
                          <div
                            key={idx}
                            className="border rounded-sm p-4 bg-indigo-50 text-indigo-900 font-medium flex flex-col justify-between"
                          >
                      ...
                      if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                          async (position) => {
                            ...
                            try {
                              ...
                              if (city) {
                                ...
                                setToast({
                                  message:
                                    "Impossible de déterminer la ville depuis votre position",
                                  type: "error",
                                });
                          ...
                          (error) => {
                            ...
                            setToast({
                              message:
                                "Impossible d'accéder à votre position. Vérifiez les permissions de géolocalisation.",
                              type: "error",
                            });
                            setTimeout(() => setToast(null), 3000);
                          },
            ...
            {hasTestResults ? (
              ...
              <div className="bg-white shadow-sm rounded-lg p-8 text-center">
                ...
                <p className="text-gray-500 mb-4">
                  Vous devez d'abord passer le test d'orientation pour voir les
                  établissements correspondant à vos métiers suggérés.
                </p>
                <button
                  onClick={() => navigate("/test")}
            ...
            <button
              onClick={() => {
                if (!hasTestResults) {
                  setToast({
                    message:
                      "Vous devez d'abord passer le test d'orientation pour accéder à la carte",
                    type: "info",
                  });
                  setTimeout(() => setToast(null), 3000);
                  return;
              ...
              title={
                !hasTestResults ? "Passez d'abord le test d'orientation" : ""
              }
            >
