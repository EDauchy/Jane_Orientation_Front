...
export function MenuItem({
  ...
  const base = `flex items-center gap-3 w-full px-4 py-2.5 text-sm font-medium transition-colors text-left ${
    disabled
...
export default function AppointmentMenu({
    ...
    sections.push([
      {
        icon: (
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[apt.status] ?? "bg-gray-400"}`}
          />
        ),
      ...
      section.push(
        {
          ...
          onClick: () => {
            ...
            onOpenRescheduleModal(
              apt.id,
              apt.proposed_date!,
              apt.user_b?.user_b_details?.availability,
              isUserA,
            );
          },
        },
  ...
  if (
    ...
    if (apt.has_review && apt.review) {
      const stars =
        "★".repeat(apt.review.rating) + "☆".repeat(5 - apt.review.rating);
      reviewSection.push({
        icon: <MessageSquare className="w-4 h-4" />,
        label: apt.review.comment
          ? `Votre avis (${stars}) — "${apt.review.comment}"`
          : `Avis déposé (${stars})`,
        variant: "info",
        disabled: true,
      });
  ...
  if (
    ...
    reviewSection.push({
      icon: <MessageSquare className="w-4 h-4" />,
      label: apt.review.comment
        ? `Avis reçu (${stars}) — "${apt.review.comment}"`
        : `Avis reçu (${stars})`,
      variant: "info",
      disabled: true,
    });
  ...
  if (apt.status === "PENDING" && isUserA) {
    destructiveSection.push({
      ...
      onClick: () => {
        ...
        onOpenConfirmDeleteModal(
          apt.id,
          `Supprimer le rendez-vous avec ${proName} le ${dateStr} ?`,
          "Supprimer",
        );
      },
    });
