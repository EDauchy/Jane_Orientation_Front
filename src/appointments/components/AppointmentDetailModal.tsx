...
export default function AppointmentDetailModal({
  ...
  const otherProfession = isUserB(otherParty)
    ? otherParty.user_b_details.profession
    : undefined;
  const otherExperience = isUserB(otherParty)
    ? otherParty.user_b_details.years_experience
    : undefined;
          ...
          <div className="relative bg-primary px-6 pt-6 pb-10">
            ...
            {otherProfession && (
              <p className="text-white/75 text-sm mt-1">
                ...
                {otherExperience && (
                  <span className="ml-2">
                    · {otherExperience} ans d'expérience
                  </span>
                )}
              </p>
            )}
            ...
            {/* Status pill */}
            <div
              className={`absolute -bottom-4 left-6 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold shadow-md ${status.classes}`}
            >
              {status.icon}
              {status.label}
            </div>
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-gray-500">
                    à{" "}
                    {new Date(apt.proposed_date).toLocaleTimeString("fr-FR", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
                  <span className="text-xs text-orange-600 italic mt-0.5">
                    {apt.proposed_by === currentUserId
                      ? "Vous avez proposé cette date"
            ...
            {apt.has_review && apt.review && (
              <div className="border-t border-gray-100 pt-5">
                <Row
                  ...
                  label={isUserA ? "Votre avis" : "Avis client"}
                >
