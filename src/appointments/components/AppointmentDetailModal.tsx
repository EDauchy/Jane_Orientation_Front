...
export default function AppointmentDetailModal({
    ...
    const formattedTime = aptDate.toLocaleTimeString('fr-FR', {
    ...
    return (
        <Modal open={open} onClose={onClose} aria-labelledby="apt-detail-title">
            <Box sx={modalStyle}>
                <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="relative bg-primary px-6 pt-6 pb-10">
                        ...
                        <AppointmentMenu appointment={apt} currentUserId={currentUserId} onOpenConfirmDeleteModal={onOpenConfirmDeleteModal} onOpenRescheduleModal={onOpenRescheduleModal} onOpenReviewModal={onOpenReviewModal} onUpdateStatus={onUpdateStatus} />
                        ...
                        {apt.user_b?.user_b_details?.profession && (
                            <p className="text-white/75 text-sm mt-1">{apt.user_b.user_b_details.profession}</p>
                        )}

                        {/* Status pill — floats at the bottom of the header */}
                    ...
                    <div className="px-6 pt-8 pb-6 space-y-5">
                        ...
                        {/* Profession (user_b details) */}
                        {apt.user_b?.user_b_details?.profession && (
                            <Row icon={<Briefcase className="w-4 h-4" />} label="Profession">
                                {apt.user_b.user_b_details.profession}
                                {apt.user_b.user_b_details.years_experience && (
                                    <span className="text-gray-400 ml-2">
                                        · {apt.user_b.user_b_details.years_experience} ans d'expérience
                                    </span>
                                )}
                            </Row>
                        )}
                            ...
                            <Row icon={<RefreshCw className="w-4 h-4 text-orange-500" />} label="Nouvelle date proposée">
                                <div className="flex flex-col gap-0.5">
                                    <span className="capitalize">
                                        {new Date(apt.proposed_date).toLocaleDateString('fr-FR', {
                                            weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
                                        })}
                                    </span>
