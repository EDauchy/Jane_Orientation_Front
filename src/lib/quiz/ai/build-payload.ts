...
export function buildAiPayload(state: AssessmentState): AiPayload {
  ...
  const cognitive: AiCognitiveSignals = {
    decisionStyle: safeString(signals.multipleChoice?.decisionStyle ?? null, 'unknown') as AiCognitiveSignals['decisionStyle'],
    fluencyBucket: safeString(signals.alternativeUses?.fluencyBucket ?? null, 'unknown') as AiCognitiveSignals['fluencyBucket'],
    divergenceBucket: safeString(signals.alternativeUses?.divergenceBucket ?? null, 'unknown') as AiCognitiveSignals['divergenceBucket'],
    budgetDominantPhase: safeString(signals.budget?.dominant ?? null, 'unknown'),
    ...
    overallTextTone: safeString(signals.openQuestions?.overallTone ?? null, 'unknown') as AiCognitiveSignals['overallTextTone'],
  };

  const riasecScores: Record<RiasecLetter, number> = signals.riasec?.scores ?? {
    R: 0, I: 0, A: 0, S: 0, E: 0, C: 0,
  };
  const sortedByScore = RIASEC_LETTERS
    .map((letter) => ({ letter, score: riasecScores[letter] }))
    .sort((a, b) => {
  ...
  const workContext: AiWorkContextSignals = {
    ...
    mode: safeString(signals.workContext?.mode ?? null, 'unknown') as AiWorkContextSignals['mode'],
    summary: safeString(signals.workContext?.summary ?? null, 'unknown') as AiWorkContextSignals['summary'],
    incomeStability: safeString(signals.risk?.incomeStability ?? null, 'unknown') as AiWorkContextSignals['incomeStability'],
    workSolitude: safeString(signals.risk?.workSolitude ?? null, 'unknown') as AiWorkContextSignals['workSolitude'],
    hierarchy: safeString(signals.risk?.hierarchy ?? null, 'unknown') as AiWorkContextSignals['hierarchy'],
    buildVsJoin: safeString(signals.risk?.buildVsJoin ?? null, 'unknown') as AiWorkContextSignals['buildVsJoin'],
  ...
  const behavioral: AiBehavioralSignals = {
    ...
    stressResponse: safeString(signals.multipleChoice?.stressResponse ?? null, 'unknown'),
    satisfactionSource: safeString(signals.multipleChoice?.satisfactionSource ?? null, 'unknown'),
    rhythmTopChoice: safeString(signals.multipleChoice?.rhythmTopChoice ?? null, 'unknown'),
  ...
  const textualResponses: AiTextualResponses = {
    openQuestions: (state.answers.openQuestions ?? []).map((q) => {
      const qSig = signals.openQuestions?.byQuestion[q.questionId as OpenQuestionId];
      return {
        questionId: q.questionId,
        text: q.text,
        wordCount: qSig?.wordCount ?? q.text.trim().split(/\s+/).filter(Boolean).length,
      ...
      state.answers.textMemo && state.answers.textMemo.mode === 'text'
        ? {
            ...
            wordCount:
              ...
              state.answers.textMemo.content.trim().split(/\s+/).filter(Boolean).length,
          }
        : null,
