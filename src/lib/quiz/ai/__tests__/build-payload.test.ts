...
describe("buildAiPayload", () => {
  ...
  it("every declared string field is a string (never null)", () => {
    ...
    const expectedStringFields: Array<[string, unknown]> = [
      ...
      [
        "cognitive.divergenceBucket",
        payload.signals.cognitive.divergenceBucket,
      ],
      [
        "cognitive.budgetDominantPhase",
        payload.signals.cognitive.budgetDominantPhase,
      ],
      ...
      [
        "workContext.incomeStability",
        payload.signals.workContext.incomeStability,
      ],
      ...
      [
        "behavioral.satisfactionSource",
        payload.signals.behavioral.satisfactionSource,
      ],
      [
        "behavioral.rhythmTopChoice",
        payload.signals.behavioral.rhythmTopChoice,
      ],
      [
        "behavioral.socialDiversityBucket",
        payload.signals.behavioral.socialDiversityBucket,
      ],
      [
        "behavioral.socialAffectBalance",
        payload.signals.behavioral.socialAffectBalance,
      ],
  ...
  it("exposes tradeoffs as a flat array of {pairId, choice, regretForOther}", () => {
    ...
    expect(["A", "B"]).toContain(
      payload.signals.behavioral.tradeoffs[0].choice,
    );
    expect(typeof payload.signals.behavioral.tradeoffs[0].regretForOther).toBe(
      "number",
    );
  });
