...
import {
  analyzeAlternativeUses,
  type AlternativeUsesSignals,
...
import {
  analyzeOpenQuestions,
  type OpenQuestionSignals,
} from "./open-question";
...
export function computeAllSignals(state: AssessmentState): AllSignals {
  ...
  const values = answers.valuesRanking
    ? analyzeValues(answers.valuesRanking)
    : null;
  const riasec = answers.riasec ? analyzeRiasec(answers.riasec) : null;
  const risk = answers.riskTolerance
    ? analyzeRisk(answers.riskTolerance)
    : null;
...
export function computeWorkContext(
  ...
  if (tradeoff) {
    ...
    else if (tradeoff.byPair["employee-vs-freelance"]?.choice === "A")
      score -= 15;
  }
