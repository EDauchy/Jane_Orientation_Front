...
describe("analyzeAlternativeUses", () => {
  ...
  it("buckets fluency correctly", () => {
    expect(analyzeAlternativeUses(mk(["a", "b", "c"])).fluencyBucket).toBe(
      "low",
    );
    expect(analyzeAlternativeUses(mk(["a", "b", "c", "d"])).fluencyBucket).toBe(
      "medium",
    );
    expect(
      analyzeAlternativeUses(mk(["a", "b", "c", "d", "e", "f", "g"]))
        .fluencyBucket,
    ).toBe("medium");
    expect(
      analyzeAlternativeUses(mk(["a", "b", "c", "d", "e", "f", "g", "h"]))
        .fluencyBucket,
    ).toBe("high");
  });
