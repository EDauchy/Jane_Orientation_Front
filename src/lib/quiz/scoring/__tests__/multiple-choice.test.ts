...
function ans(questionId: string, value: string | string[]): MultipleChoiceAnswer {
...
describe('analyzeMultipleChoice', () => {
  it('maps decision styles', () => {
    expect(analyzeMultipleChoice([ans('qcm-decision', 'data')]).decisionStyle).toBe('analytical');
    expect(analyzeMultipleChoice([ans('qcm-decision', 'intuition')]).decisionStyle).toBe('intuitive');
    expect(analyzeMultipleChoice([ans('qcm-decision', 'advice')]).decisionStyle).toBe('social');
    expect(analyzeMultipleChoice([ans('qcm-decision', 'test')]).decisionStyle).toBe('empirical');
  });
  ...
  it('flexibilitySeeker from rhythm top choice', () => {
    expect(
      analyzeMultipleChoice([ans('qcm-rhythm', ['flexible', 'sprint'])]).flags.flexibilitySeeker,
    ).toBe(true);
    expect(
      analyzeMultipleChoice([ans('qcm-rhythm', ['waves', 'steady'])]).flags.flexibilitySeeker,
    ).toBe(true);
    expect(
      analyzeMultipleChoice([ans('qcm-rhythm', ['steady', 'waves'])]).flags.flexibilitySeeker,
    ).toBe(false);
  });
