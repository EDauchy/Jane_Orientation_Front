...
export type MultipleChoiceSignals = {
  ...
  decisionStyle: 'analytical' | 'intuitive' | 'social' | 'pragmatic' | 'empirical' | null;
...
export function analyzeMultipleChoice(answers: MultipleChoiceAnswer[]): MultipleChoiceSignals {
  ...
  const decisionStyle =
    typeof decisionRaw === 'string' ? (DECISION_MAP[decisionRaw] ?? null) : null;
  ...
  const satisfactionSource = typeof satisfactionRaw === 'string' ? satisfactionRaw : null;
  ...
  return {
    ...
    flags: {
      ...
      flexibilitySeeker: rhythmTopChoice === 'flexible' || rhythmTopChoice === 'waves',
    },
  };
}
