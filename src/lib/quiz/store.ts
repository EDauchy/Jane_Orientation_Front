...
function upsertById<T extends { questionId: string }>(list: T[] | undefined, item: T): T[] {
  if (!list) return [item];
...
export const useAssessment = create<StoreApi>()(
  persist(
    (set) => ({
      ...
      setAudience: (a) =>
        set((s) => ({ state: { ...s.state, audience: a, currentModuleIdx: 0 } })),
      setQualification: (q) => set((s) => ({ state: { ...s.state, qualification: q } })),

      setCurrentIdx: (idx) =>
        set((s) => {
          const total = s.state.audience ? totalInFlow(s.state.audience) : 0;
          const clamped = total === 0 ? 0 : Math.max(0, Math.min(total - 1, idx));
          return { state: { ...s.state, currentModuleIdx: clamped } };
        }),

      setBudget: (a) => set((s) => ({ state: patchAnswers(s.state, { budget: a }) })),
      setAlternativeUses: (a) =>
        set((s) => ({ state: patchAnswers(s.state, { alternativeUses: a }) })),
      setTradeoff: (a) => set((s) => ({ state: patchAnswers(s.state, { tradeoff: a }) })),
      ...
      setRiasec: (a) => set((s) => ({ state: patchAnswers(s.state, { riasec: a }) })),
      setRiskTolerance: (a) =>
        set((s) => ({ state: patchAnswers(s.state, { riskTolerance: a }) })),
      setSocialNuance: (a) =>
        set((s) => ({ state: patchAnswers(s.state, { socialNuance: a }) })),
      setTextMemo: (a) => set((s) => ({ state: patchAnswers(s.state, { textMemo: a }) })),
    }),
    {
      ...
      migrate: (persisted, version) => {
        const incoming = persisted as { state?: { schemaVersion?: string } } | null;
        if (
          version !== STORE_VERSION ||
